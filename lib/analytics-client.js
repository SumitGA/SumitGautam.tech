...
/* Development does not write events.
 *
 * .env.local points at the same Supabase project as production, so every
 * `npm run dev` session was adding real rows to the table the dashboard reads
 * — inflating pageviews with paths nobody visited and skewing every rate
 * computed against sessions. The data is only worth having if it describes
 * visitors.
 *
 * NODE_ENV is the check rather than the hostname: a production build served
 * from localhost is still exercising the real path, which is what you want
...
export function track(event, { path, meta } = {}) {
  ...
  if (!ENABLED) {
    if (!warned) {
      warned = true;
      console.info("[analytics] disabled in development — no events are being recorded");
    }
    return;
  }
