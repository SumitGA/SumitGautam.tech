...
/* What the AI assistant is told about the writing.
 *
 * Two shapes, for one reason: the system prompt is rebuilt every ten minutes
 * and sent with EVERY message, so anything in it is paid for on every turn and
 * grows without limit as posts accumulate. Full content does not scale —
 * see docs/assistant-context-budget.md for the numbers.
 *
 *  * Every post contributes title, excerpt and section headings. Headings are
 *    the load-bearing part: "Act III: The Execution" tells the model what was
 *    covered precisely enough to answer specifically and link, at roughly a
 *    hundred tokens a post.
 *
 *  * The newest few also contribute full content, because recent work is what
 *    gets asked about and the detail is worth the tokens for a bounded number
 *    of posts.
 *
 * The cost is therefore flat in the number of posts, not linear.
 */
export async function getPostsForPrompt({ fullCount = 3 } = {}) {
  const supabase = db();
  if (!supabase) return [];

  const { data, error } = await supabase
    .from("posts")
    .select("slug,title,excerpt,content,published_at,series,series_order")
    .order("published_at", { ascending: false });

  if (error) {
    console.error("[blog] getPostsForPrompt:", error.message);
    return [];
  }
  ...
  return (data || []).map((post, i) => ({
    ...
    headings: headingsOf(post.content),
    // Only the most recent few carry their body.
    content: i < fullCount ? post.content : null,
  }));
}

/* Section headings, without pulling in the markdown renderer — this runs in a
   serverless function on a cold start and does not need Shiki. Fenced blocks
   are skipped so a "# comment" inside code is not read as a heading. */
