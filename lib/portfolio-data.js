...
/** A project earns a detail page once it has a slug and some written content. */
function hasCaseStudy(r) {
  return !!(r.slug && (r.problem || r.approach || r.outcome));
}

function mapProject(r, i) {
  return {
    id: String(r.id ?? i),
    name: r.name,
    url: r.url,
...
export async function getProjects() {
  ...
  return { data: rows.map(mapProject) };
}

/** Single project by slug — used by /projects/[slug]. Null when not found. */
export async function getProjectBySlug(slug) {
  ...
  if (error || !data || !hasCaseStudy(data)) return null;
  return mapProject(data, 0);
}

/** Slugs of every project with a case study — used to prerender detail pages. */
export async function getCaseStudySlugs() {
  const supabase = await db();
  if (!supabase) return [];
  const { data, error } = await supabase.from("projects").select("*");
  if (error || !data) return [];
  return data.filter(hasCaseStudy).map((r) => r.slug);
}
...
/* Whether anything is actually published, so the nav can omit a Blog link that
   leads to an empty page. RLS already excludes drafts and future-dated posts,
   so this counts exactly what a visitor would see. Head-only: no rows are
